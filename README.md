# Moltbook Report Podcast

AI-generated podcast exploring the Moltbook Report project history, created with NotebookLM.

## 🎧 Listen

- **Web Player**: https://ethanjstark.github.io/moltbook-podcast/
- **RSS Feed**: https://ethanjstark.github.io/moltbook-podcast/feed.xml

## 📱 Subscribe

### Apple Podcasts
1. Open Apple Podcasts
2. File → Add a Podcast by URL
3. Paste: `https://ethanjstark.github.io/moltbook-podcast/feed.xml`

### Other Apps
Copy the RSS feed URL above into your preferred podcast app.

## 📝 Episodes

### Episode 1: AI Agents Speedrun Civilization on Moltbook (Feb 1, 2026)
Exploring how AI agents build and speedrun civilization on the Moltbook platform.

[Listen on web](https://ethanjstark.github.io/moltbook-podcast/) | [Direct download](https://github.com/EthanJStark/moltbook-podcast/releases/download/v0.1.0/20260201-ai-agents-speedrun-civilization.mp3) | [Read Transcript](https://ethanjstark.github.io/moltbook-podcast/transcript-episode-1.html)

## 🔧 Technical Details

- **Hosting**: GitHub Pages
- **Audio CDN**: GitHub Releases
- **Feed Format**: RSS 2.0 with iTunes extensions
- **Audio Format**: MP3, 128kbps, 44.1kHz

## 📚 Documentation

- [Adding Episodes](ADDING_EPISODES.md) - Guide for adding new episodes
- [Validation Results](VALIDATION.md) - Feed validation status

## 🤖 Generated With

- [NotebookLM](https://notebooklm.google/) - AI podcast generation
- [GitHub Pages](https://pages.github.com/) - Free static site hosting

## 📄 License

Audio content: Creative Commons Attribution 4.0
Website code: MIT License
